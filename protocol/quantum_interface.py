...
STILL PROVISIONAL, BUT NOW BACKED
---------------------------------
These names are M2's, not contracts.py's, so they can change without a
Decision Log entry. As of the P1 protocol selection all five are implemented
by `core.runtime` and `core.pauli`.

`teleport_and_measure` is the one the signature protocol actually runs;
`teleport` and `measure` remain because they are cheaper when only one half
of the process is needed, and because M3 already uses the teleport path.
...
knows what a *signature* is belongs in signer.py or verifier.py. In
particular the (basis, bit) pairs handed to `teleport_and_measure` are Pauli
eigenstates -- a physical description -- not "signature elements".
"""
...
class QuantumCore(Protocol):
    ...
    def teleport_and_measure(
        ...
        *,
        ...
        The method the signature protocol actually runs. `preparations[i]` is
        the (basis, bit) Pauli eigenstate the signer puts on the message
        qubit -- bit 0 for the +1 eigenstate, per
        contracts.MeasurementRecord. `bases[i]` is the basis the recipient
        measures in.

        M2 expects: `((clbit0, clbit1), measured_bit)` per copy, in copy
        order, both halves FROM THE SAME SHOT.

        That last requirement is the whole reason this method exists rather
        than M2 calling `teleport` then `measure`. Those are independent
        runs, so their outputs describe different copies: the Bell outcome
        would not be the one whose Pauli correction produced that measured
        bit. Legitimate signatures would then show a ~50% mismatch rate and
        the protocol would reject everything -- a failure that looks like
        broken physics rather than a broken call sequence.
        """
        ...
...
class M1QuantumCore:
    ...
    Owns no logic -- every method delegates to core.runtime or core.pauli.
    `core.*` is imported lazily so `import protocol` still works without
    Qiskit/Aer; *calling* these methods requires them.

    `seed` is the Aer simulator seed, carried here rather than passed per
    call so that the QuantumCore interface stays free of simulator detail --
    a hardware backend would have no seed to take. Each copy in a batch is
    run at `seed + copy_index`, so a whole batch is reproducible without
    every copy sharing one draw. Leave it None for nondeterministic runs;
    set it in tests so a failure can be replayed.
    """
    ...
    def measure(
        ...
        return run_measure_bits(resource, basis, noise_level=noise_level, seed=self.seed)
    ...
    def teleport_and_measure(
        ...
        *,
        ...
        from core.runtime import run_teleport_and_measure

        return run_teleport_and_measure(
            resource, preparations, bases, noise_level=noise_level, seed=self.seed
        )
